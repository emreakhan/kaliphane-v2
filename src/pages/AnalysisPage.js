...
import { 
    ...
    CalendarDays, AlertOctagon, History 
} from 'lucide-react';
...
// --- GÜNCELLENMİŞ: ANALİZ SAYFASI (V2.8.1 - Yetkiler Genişletildi) ---
const AnalysisPage = ({ projects, personnel, loggedInUser }) => {
    ...
    // Artık sadece Admin değil, Tasarımcı ve Yetkili de bu sekmeyi görebilir.
    ...
    // --- 4. SEKME: HATA / REWORK ANALİZİ MANTIĞI ---
    const reworkAnalysis = useMemo(() => {
        if (!canViewReworkTab) return null; // Yetki kontrolü güncellendi

        // 1. Tüm Hata Kayıtlarını Düzleştir
        ...
        const moldCounts = {};
        allReworks.forEach(r => { moldCounts[r.moldName] = (moldCounts[r.moldName] || 0) + 1; });
        const mostProblematicMold = Object.entries(moldCounts).sort((a,b) => b[1] - a[1])[0];

        const reasonCounts = {};
        allReworks.forEach(r => { reasonCounts[r.reason] = (reasonCounts[r.reason] || 0) + 1; });
    ...
    // --- DİĞER SEKMELERİN MANTIKLARI (Aynı) ---
    ...
    // --- HATA ANALİZ KARTI (GÜNCELLENDİ: reworkAnalysis null değilse çalışır) ---
        ...
        return (
            <div className="space-y-8 animate-fadeIn">
                 ...
                 <div className="bg-red-50 dark:bg-red-900/20 p-6 rounded-xl border border-red-200 dark:border-red-800">
                    <h3 className="text-2xl font-bold text-red-800 dark:text-red-300 flex items-center">
                        <AlertOctagon className="w-8 h-8 mr-3" />
                        Hata ve Yeniden İşleme (Rework) Kayıtları
                    </h3>
                    <p className="text-red-600 dark:text-red-400 mt-2">
                        Aşağıdaki liste, operasyon sırasında bildirilen hataları ve sıfırlanan işlemleri gösterir.
                    </p>
                </div>

                {/* Özet İstatistikler */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow border-l-4 border-red-500">
                        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Toplam Hata Kaydı</p>
                        <h4 className="text-4xl font-bold text-gray-900 dark:text-white mt-1">{totalReworks}</h4>
                    </div>
                    <div className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow border-l-4 border-orange-500">
                        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">En Sorunlu Kalıp</p>
                        <h4 className="text-xl font-bold text-gray-900 dark:text-white mt-1 truncate">
                            {mostProblematicMold ? mostProblematicMold.name : '---'}
                        </h4>
                        <p className="text-xs text-gray-400">{mostProblematicMold ? `${mostProblematicMold.count} Hata` : ''}</p>
                    </div>
                    <div className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow border-l-4 border-yellow-500">
                        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">En Sık Görülen Sebep</p>
                        <h4 className="text-xl font-bold text-gray-900 dark:text-white mt-1 truncate">
                            {mostCommonReason ? mostCommonReason.name : '---'}
                        </h4>
                        <p className="text-xs text-gray-400">{mostCommonReason ? `${mostCommonReason.count} Kez` : ''}</p>
                    </div>
                </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider dark:text-gray-300">Operasyon</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider dark:text-gray-300">Hata Nedeni</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider dark:text-gray-300">Bildiren</th>
                                </tr>
                            </thead>
                                ...
                                {data.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                            Henüz hiç hata kaydı bulunmamaktadır.
                                        </td>
                                    </tr>
                                ) : (
                                    data.map((log, idx) => (
                                        <tr key={idx} className="hover:bg-red-50 dark:hover:bg-red-900/10 transition">
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                {formatDateTime(log.date)}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900 dark:text-white">{log.moldName}</div>
                                                <div className="text-xs text-gray-500 dark:text-gray-400">{log.taskName}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900 dark:text-white font-bold">{log.opType}</div>
                                                <div className="text-xs text-gray-500">Tezgah: {log.machine}</div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                ...
                                                <div className="text-xs text-red-400 mt-1">
                                                    Silinen İlerleme: %{log.previousProgress}
                                                </div>
                                            </td>
                                        </tr>
                                    ))
    ...
    // --- ARAYÜZ BİLEŞENLERİ (Diğer Sekmeler Aynı) ---
    ...
    return (
        <div className="p-4 bg-white dark:bg-gray-800 rounded-xl shadow-xl min-h-[85vh]">
            ...
            <div className="flex space-x-4 border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto">
                ...
                {/* HATA ANALİZİ BUTONU (YETKİ KONTROLLÜ) */}
                {canViewReworkTab && (
                    <button
                        onClick={() => { setActiveTab('rework'); setSelectedId(null); setSearchTerm(''); }}
                        className={`py-2 px-4 font-medium text-sm border-b-2 transition-colors whitespace-nowrap ${
                            activeTab === 'rework'
