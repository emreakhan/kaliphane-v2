...
import { UserCircle, Star, Clock, Calendar as CalendarIcon, CheckCircle, XCircle, AlertTriangle, TrendingUp, Award } from 'lucide-react';
import { DESIGN_JOB_STATUS, PERSONNEL_ROLES } from '../config/constants.js';

// --- TATİL VE AKILLI SAYAÇ KESİNTİ ALGORİTMASI ---
...
// Start ile End tarihi arasındaki NET çalışma saatini hesaplar (18:00 ve haftasonu kırpılmış haliyle)
const calculateNetWorkingHours = (startMs, endMs, autoPause = true) => {
    if (!autoPause) {
        return Math.max(0, (endMs - startMs) / (1000 * 60 * 60)); // Fazla mesaideyse ham saati dön
    }
    ...
    while (current < end) {
        const currentDayStart = new Date(current); currentDayStart.setHours(0, 0, 0, 0);
        const currentDayEnd = new Date(current); currentDayEnd.setHours(23, 59, 59, 999);

        const dayEnd = end < currentDayEnd ? end : currentDayEnd;

        // Tatil değilse saatleri hesapla
        if (current.getDay() !== 0 && current.getDay() !== 6 && !isHoliday(current)) {
            const workStart = new Date(current); workStart.setHours(8, 0, 0, 0);
            const workEnd = new Date(current); workEnd.setHours(18, 0, 0, 0);

            const effectiveStart = current > workStart ? current : workStart;
            const effectiveEnd = dayEnd < workEnd ? dayEnd : workEnd;
...
const DesignPerformancePage = ({ designJobs, personnel }) => {
    ...
    const performanceData = useMemo(() => {
        ...
        const evaluatedJobs = completedJobs.map(job => {
            // AKILLI SAYAÇ İLE GERÇEK HARCANAN SÜREYİ HESAPLA
            let actualHours = 0;
            if (job.workSessions) {
                job.workSessions.forEach(ws => {
                    if (ws.startTime) {
                        const start = new Date(ws.startTime);
                        const end = ws.endTime ? new Date(ws.endTime) : new Date(job.completedAt || new Date());
                        actualHours += calculateNetWorkingHours(start, end, job.autoPause !== false);
                    }
                });
            }
            ...
            if (job.deadlineDate && job.completedAt) {
                const deadline = new Date(job.deadlineDate + 'T23:59:59'); 
                                ...
                                <div key={job.id} className="p-4 sm:p-5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition flex flex-col lg:flex-row gap-4 lg:items-center">
                                    ...
                                    <div className="flex flex-wrap lg:flex-nowrap gap-6 lg:gap-8 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700 lg:w-[450px]">
                                        <div className="flex-1">
                                            <div className="flex items-center text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5 uppercase"><Clock className="w-3.5 h-3.5 mr-1" /> Süre Hedefi</div>
                                            <div className="flex justify-between text-sm"><span className="text-gray-600 dark:text-gray-300">Hedeflenen:</span><span className="font-bold text-gray-800 dark:text-gray-200">{formatHours(job.estimatedHours)}</span></div>
