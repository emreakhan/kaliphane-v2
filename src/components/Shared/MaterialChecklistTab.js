...
import { 
    ListChecks, Plus, CheckCircle, Trash2, PackageCheck, PackageX, QrCode, FileSpreadsheet, ListPlus, Loader, Printer
} from 'lucide-react'; 
import { doc, updateDoc } from '../../config/firebase.js'; 
import { OPERATION_STATUS, MATERIAL_TYPES, PROJECT_COLLECTION } from '../../config/constants.js';
        ...
        <div style={{ display: 'none' }}>
            <div ref={ref} style={{
                ...
                <div style={{ flex: 1, paddingRight: '10px', display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center' }}>
                    ...
                    <div style={{ marginTop: 'auto', fontSize: '8px', color: '#888' }}>
                        ID: {material.id.substring(0,8).toUpperCase()} {material.erpCode ? ` | ERP: ${material.erpCode}` : ''}
                    </div>
                </div>
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Barkod / Etiket Önizleme">
            <div className="flex flex-col items-center space-y-5">
                ...
                <div className="bg-gray-200 dark:bg-gray-700 p-6 rounded-xl flex justify-center w-full overflow-auto">
                    ...
                    >
                        <div style={{ flex: 1, paddingRight: '10px', display: 'flex', flexDirection: 'column', height: '100%', justifyItems: 'flex-start' }}>
                            <div style={{ fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '2px' }}>
                                {customer || 'MÜŞTERİ BELİRTİLMEDİ'}
                            </div>
                            <div style={{ fontSize: '16px', fontWeight: '900', marginBottom: '4px', textTransform: 'uppercase', lineHeight: '1.1' }}>
                                {moldName}
                            </div>
                            ...
                            <div style={{ fontSize: '12px', fontWeight: 'bold' }}>
                                Ölçü: {material.dimensions || 'Belirtilmedi'}
                            </div>
                            
                            <div style={{ marginTop: 'auto', fontSize: '9px', fontWeight: 'bold' }}>
                                ID: {material.id.substring(0,8).toUpperCase()} {material.erpCode ? `| ERP: ${material.erpCode}` : ''}
                            </div>
                        </div>
                ...
                <div className="flex justify-end gap-3 w-full border-t dark:border-gray-700 pt-4">
                    <button onClick={onClose} className="px-5 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 rounded-lg font-bold transition text-sm">
                        İptal
                    </button>
                    <button onClick={handlePrint} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-md transition flex items-center text-sm">
...
const MaterialChecklistTab = ({ mold, materials, canManageMaterials, loggedInUser, db }) => {
    ...
    const handleAddMaterial = async () => {
        ...
        try {
            ...
            const updatedMaterials = [...materials, materialEntry];
    ...
    const handleDeleteMaterial = async (materialId) => {
        if(!window.confirm("Bu malzemeyi listeden silmek istediğinize emin misiniz?")) return;
        try {
            const updatedMaterials = materials.filter(m => m.id !== materialId);
            const moldRef = doc(db, PROJECT_COLLECTION, mold.id);
            await updateDoc(moldRef, { materials: updatedMaterials });
        } catch (error) { console.error("Malzeme silme hatası:", error); }
    };
    ...
    const handleUpdateMaterialStatus = async (materialId, newStatus) => {
        ...
        try {
            const updatedMaterials = materials.map(m => {
                if (m.id === materialId) {
                    return { ...m, status: newStatus, statusUpdatedAt: getCurrentDateTimeString(), statusUpdatedBy: loggedInUser.name };
                }
                return m;
            });
            const moldRef = doc(db, PROJECT_COLLECTION, mold.id);
    ...
    const handleOpenBarcodePreview = (material) => {
        setSelectedMaterialForPrint(material);
        setIsBarcodePreviewOpen(true);
    };

    // --- .XLSX EXCEL OKUMA FONKSİYONU ---
    const handleExcelFileUpload = (e) => {
        ...
        reader.onload = (event) => {
            ...
            jsonData.forEach(row => {
                ...
                const stokAdi = obj['stokadi'] || obj['stok_adi'] || obj['stok adi'] || obj['isim'] || obj['name'] || '';
                if (!stokAdi) return; // İsim yoksa atla

                const miktar = parseInt(obj['imiktar'] || obj['miktar'] || obj['adet'] || obj['qty'] || 1);
                
                let dims = [];
                if (obj['en'] && obj['en'] !== '0') dims.push(`E:${obj['en']}`);
                if (obj['boy'] && obj['boy'] !== '0') dims.push(`B:${obj['boy']}`);
                if (obj['kalinlik'] && obj['kalinlik'] !== '0' && obj['kalinlik'] !== '0.0') dims.push(`K:${obj['kalinlik']}`);
                if (obj['iccap'] && obj['iccap'] !== '0') dims.push(`İÇ:${obj['iccap']}`);
                if (obj['discap'] && obj['discap'] !== '0') dims.push(`DIŞ:${obj['discap']}`);
                
                const dimensionsStr = dims.length > 0 ? dims.join(' x ') : (obj['olculer'] || obj['ölçüler'] || obj['ebat'] || '');

                const nameUpper = stokAdi.toUpperCase();
                let type = MATERIAL_TYPES.STANDART_ELEMAN;
                if (nameUpper.includes('ÇELİK') || nameUpper.includes('CELIK') || nameUpper.includes('ÇELİĞİ')) {
                    type = MATERIAL_TYPES.CELIK;
                }

                parsedData.push({
                    name: stokAdi,
                    erpCode: obj['stokkodu'] || obj['stok_kodu'] || obj['stok kodu'] || '',
                    type: type,
            ...
            if (parsedData.length > 0) {
                setErpPreviewData(parsedData);
                setIsErpModalOpen(true);
            } else {
                alert("Excel dosyası okundu ancak uygun veri (Stok Adı, Miktar vb.) bulunamadı.");
            }
        };
        reader.readAsArrayBuffer(file);
        e.target.value = null; // Input sıfırlama
    };
    ...
    const handleSaveErpData = async () => {
        ...
        try {
            ...
            const updatedMaterials = [...materials, ...newEntries];
            const moldRef = doc(db, PROJECT_COLLECTION, mold.id);
            await updateDoc(moldRef, { materials: updatedMaterials });
            ...
            console.error("ERP Kayıt Hatası:", error);
        ...
        <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                    ...
                    Siparişi Verilen / Gelen Malzemeler
                </h3>
                ...
                {canManageMaterials && (
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        <label className="flex items-center justify-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-lg shadow-md transition cursor-pointer flex-1 md:flex-none">
                            <FileSpreadsheet className="w-4 h-4 mr-2" /> ERP'den Yükle (.xlsx)
                            <input 
                                type="file" 
                                accept=".xlsx, .xls, .csv" 
                                className="hidden" 
                        ...
                        <button 
                            ...
                            className="flex items-center justify-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition flex-1 md:flex-none"
                        >
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tür</th>
                            <th className="px-6 py-3 text-left text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ölçüler</th>
                            <th className="px-6 py-3 text-left text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">Adet</th>
                            <th className="px-6 py-3 text-left text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">Durum</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {materials.length === 0 ? (
                            <tr>
                                <td colSpan="6" className="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400 font-medium">
                                    Bu kalıp projesi için henüz malzeme kaydı girilmemiş.
                                </td>
                            </tr>
                        ) : (
                            materials.map(mat => (
                                <tr key={mat.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">
                                        ...
                                        <div className="text-[10px] text-gray-400 font-normal mt-0.5">
                                            ID: {mat.id.substring(0,8)}
                                            {mat.erpCode && <span className="ml-2 text-indigo-400 border border-indigo-200 dark:border-indigo-800 px-1 rounded">ERP: {mat.erpCode}</span>}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {mat.status === OPERATION_STATUS.HAMMADDE_BEKLIYOR ? (
                                            <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-orange-100 text-orange-800 flex items-center w-max">
                                                <PackageX className="w-3.5 h-3.5 mr-1" /> Siparişte / Bekliyor
                                            </span>
                                        ) : (
                                            <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-green-100 text-green-800 flex items-center w-max">
                                                <PackageCheck className="w-3.5 h-3.5 mr-1" /> Depoda Mevcut
                                            </span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end gap-2">
                                            {canManageMaterials && mat.status === OPERATION_STATUS.HAMMADDE_BEKLIYOR && (
                                                <button 
                                                    onClick={() => handleUpdateMaterialStatus(mat.id, OPERATION_STATUS.DEPODA)}
                                                    className="p-1.5 bg-green-50 text-green-600 hover:bg-green-100 dark:bg-green-900/30 dark:hover:bg-green-900/50 rounded transition"
                                                    title="Depoya Alındı Olarak İşaretle"
                                                >
                                                    <CheckCircle className="w-4 h-4" />
                                                </button>
                                            )}
                                            
                                            {mat.status === OPERATION_STATUS.DEPODA && (
                                                <button 
                                                    onClick={() => handleOpenBarcodePreview(mat)}
                                                    className="p-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 rounded transition flex items-center"
                                                    title="QR Kodlu Etiket Önizleme ve Yazdır"
                                                >
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">
                                Malzeme / Parça Adı <span className="text-red-500">*</span>
                            </label>
                            <input 
                                type="text" 
                                value={newMaterial.name} 
                                onChange={(e) => setNewMaterial({...newMaterial, name: e.target.value})}
                                placeholder="Örn: 2738 Dişi Çekirdek Çeliği"
                                className="w-full p-2.5 border rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-sm"
                            />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    placeholder="Örn: E:400 B:500 K:120"
                                    className="w-full p-2.5 border rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-sm"
                                />
                            </div>
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full p-2.5 border rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-sm text-center"
                                />
                            </div>
                        </div>
                        ...
                        <div className="pt-4 flex justify-end gap-3 border-t dark:border-gray-700">
                            ...
                            <button 
                                ...
                                disabled={isSaving || !newMaterial.name || !newMaterial.quantity} 
                            ...
                            >
                                {isSaving ? 'Ekleniyor...' : 'Listeye Ekle'}
                            </button>
                        </div>
                    </div>
                </Modal>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-4xl max-h-[80vh] shadow-2xl flex flex-col overflow-hidden">
                        <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
                            <h2 className="text-lg font-black text-gray-800 dark:text-white flex items-center">
                                <ListPlus className="w-5 h-5 mr-2 text-emerald-600" />
                                ERP Verisi Önizleme ({erpPreviewData.length} Kayıt)
                            </h2>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead className="bg-gray-200 dark:bg-gray-700 sticky top-0 shadow-sm z-10">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 text-left text-xs font-black text-gray-600 dark:text-gray-300">Ölçüler (E x B x K)</th>
                                    ...
                                    {erpPreviewData.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <td className="px-4 py-2">
                                                ...
                                                {item.erpCode && <div className="text-[10px] text-indigo-500">{item.erpCode}</div>}
                                            </td>
                                            <td className="px-4 py-2">
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${item.type === MATERIAL_TYPES.CELIK ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800'}`}>
                                                    {item.type}
                                                </span>
                                            </td>
                        ...
                        <div className="p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button 
                                    ...
                                    disabled={isSaving} 
                                    className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg shadow-md transition disabled:opacity-50 text-sm flex items-center"
                                >
                                    {isSaving ? <Loader className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                                    Onayla ve Ekle
                                </button>
                            </div>
                        </div>
            ...
            <BarcodePreviewModal 
                ...
                material={selectedMaterialForPrint} 
